...
`swift test --no-parallel` runs **200 tests, 0 skipped, 0 failures** (the Metal kernels skip
only on a host with no GPU, which is why CI runners report skips), with **398** standard-library Python
tests run in CI.
