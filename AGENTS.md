...
at **117 tests, 2 skipped, 0 failures** (the skips are the Metal kernel tests, which
...
**M2 has started**: its deterministic reduction contract (`D17`) is decided and demonstrated on the
fixture's real weights, with the transport still to build. **M3–M5 have not started**.
...
- **The public status of this repository has swung three times, and only the latest is
  ...
  build is clean and **117 Swift tests pass**, while on the `macos-26` CI image (Xcode
