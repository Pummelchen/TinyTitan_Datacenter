...
at **144 tests, 2 skipped, 0 failures** (the skips are the Metal kernel tests, which
...
(`D19`) and the shard plan as data (`D20`) are implemented and demonstrated over a socket pair — a
two-node exchange driven by a **loaded plan file** is bit-identical to the single-node forward, a resend
merges, and a missing term fails the run. What remains is bring-up (`DC-042`) and a real second process
on a real link (`DC-045`), which needs the cluster. **M3–M5 have not started**.
...
- **The public status of this repository has swung three times, and only the latest is
  ...
  build is clean and **144 Swift tests pass**, while on the `macos-26` CI image (Xcode
