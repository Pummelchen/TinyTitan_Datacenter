...
> **CLOSED by `D295`, with the arithmetic.** The co-occurrence structure is real - mean lift 1.941, maximum 305x, ~4,300 pairs per layer above 2x chance - and **a balanced partition built from it is worth only +10.9% over a balanced marginal split** (2.58 groups touched against 2.89; random is 3.62). An unbalanced partition reaches 2.19, and that gap is the imbalance artifact rather than usable concentration. **It is a ~1.1x on the serving cost against a target that needs 2.8x, so it is not worth building.** Two further results: layers have **no** cross-layer correlation (adjacent overlap 1.01x chance), so the unit of placement is a single layer and there is no cross-layer prefetch; and the marginal distribution is near-uniform, which does **not** imply the joint is - the earlier withdrawal tested the wrong statistic. Full history in `D290`, `D292`, `D294`, `D295`.
>
> **Reopened (superseded) by `D292`.**, which are statistics of the **marginal** distribution. The operator's idea was about **co-activation**, and the two are independent. The test that does measure it gives a **mean lift of 1.941, a median of 1.060 and a maximum of 305x, with ~4,300 pairs per layer above 2x chance** - structure, in a small population of strongly correlated pairs. A **marginal-greedy** split still leaves a token touching 3.14 of 4 groups, which does not exploit it; **a partition built from the affinity graph has not been tried.** The original withdrawal follows.
>
...
**Measurement 1 is now POSITIVE, on the right shape (`D302`).** Six probes were needed and the first four were mine to get wrong: a matmul and three conv formulations all showed the ANE-eligible unit slower, because **all four tested a linear projection**, and the ANE does not serve projections. At a full 4,096-token chunk the same toolchain gives opposite results by shape family:
...
**So the ANE does engage, and on attention at a full chunk it is 1.9x the GPU path Core ML offers.** The operating point is upstream's and now measured: a **4,096-token chunk**, a one-time Core ML compile paid once per process, and **`used_ane`** checked because no speedup may be quoted from an arm that fell back to the GPU. **And the block does not repeat the op-level win (`D303`).** QK^T + softmax + PV at 4,096 gives **1,471.3 GFLOP/s on `CPU_AND_GPU` against 1,206.8 on `ALL`** - the ANE-eligible path is **1.22x slower** on the composite where it was **1.9x faster** on QK^T alone. It still beats `CPU_ONLY` by 1.9x, so it is not falling back, and the difference between the two probes is the softmax and the second matmul. **The likely reading is that softmax is the op an acceleration engine handles badly**, which would also explain why upstream moves the block as a *unit* rather than as a set of ops. **So: one positive op-level result, no block-level one, and the boundary is now firm.**

This is 7% of the M2's ~15.8 TOPS, so it evidences engagement rather than good use, and one op is not the full-attention block the sister project moves - the 1.9x is a floor on the op, not a measured block speedup.**

 A matmul on `ComputeUnit.ALL` showed no engagement (1.27 ms against 1.20 on CPU). A bare 1x1 conv on `ALL` was 50x slower than CPU. A repeated conv appeared to show free arithmetic - **and that was my own test being optimised away**, because all K convolutions took the same input and weight. Chained so nothing can fold, `ALL` is **still slower than the CPU at every size** (156.57 ms against 34.56 ms at K=32, both doing 42.95 GFLOP) and its GFLOP/s rises with K, which is a fixed cost being amortised rather than free compute. **The CPU is the fastest Core ML unit measured here, at ~1.2 TFLOP/s.** 274 GFLOP/s is an order of magnitude below the M2's ~15.8 TOPS, so **the ANE was never demonstrably selected in any of the four probes.** The route is not reachable from Core ML by any formulation tried; the next step is to study how the sister project built its ANE model rather than probing further from first principles.

 A prefill-shaped MIL matmul - `[1,128,2048] x [2048,2560]`, 1.34 GFLOP - converted to fp16 and run twenty times gives **1.27 ms on `ComputeUnit.ALL` and 1.20 ms on `CPU_ONLY`**: the ANE-eligible path is *slower*, so the Neural Engine was not used. 1.1 TFLOPS is ~7% of the M2's ~15.8 TOPS, and the traffic is only 8.75 GB/s, so the op is compute-bound at a rate no ANE produces. **The toolchain works and converts this shape; the ANE is not reachable by writing a matmul and asking for `ALL`.** It wants a shape it was built for, and in practice a whole converted network - which is what the sister project must have done. **No ANE benefit has been measured.**
