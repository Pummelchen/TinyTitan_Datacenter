...
**So the ANE does engage, and on attention at a full chunk it is 1.9x the GPU path Core ML offers.** The operating point is upstream's and now measured: a **4,096-token chunk**, a one-time Core ML compile paid once per process, and **`used_ane`** checked because no speedup may be quoted from an arm that fell back to the GPU. **This is 7% of the M2's ~15.8 TOPS, so it evidences engagement rather than good use, and one op is not the full-attention block the sister project moves - the 1.9x is a floor on the op, not a measured block speedup.**
