...
## D378 — Four full-pair runs fail and six probes succeed, and the discriminator is the invocation rather than any variable in it

**The tally, after retrying the real run twice more:**

    full pair runs (both stages, all four edges configured)   0 of 4 connected, A errored every time
    probe runs (one edge configured, one stage)               6 of 6 connected

**And the invocations differ in nothing that five rounds of inspection has been able to name.** The probes and the
full runs have used the same binary, the same nodes, the same addresses, the same ports, both prompts, both
generation lengths, both peer configurations and both orderings. **The one thing that separates them is that in the
full run *both stages are fully configured and both edges exist*** - and every attempt to reduce that to a single
variable has produced a conclusion the next measurement withdrew.

**So this is where process-level inspection stops being the right instrument.** Five rounds have produced four
withdrawn conclusions, one invalid control, and one statistic: **the failure is perfectly correlated with the run
being the real one and never occurs in a probe.** That is not a variable, and **no further environment-variable
bisection can turn it into one** - the space has been sampled and the two populations do not overlap.

**What can see it, and why it is worth the cost.** `EHOSTUNREACH` is generated by the kernel, so **the kernel's own
record is the instrument**: a packet capture on either node during a full run, or the connection-attempt log
(`log stream --predicate 'process == "TinyTitanCLI"'` on macOS), would show whether a SYN left the machine, whether
one came back, or whether **the connection was refused before any packet existed** - which is what `D287`'s Local
Network Privacy does. **That distinction is the whole question and five rounds of `lsof` and `netstat` cannot make
it**, because both look identical from the process's side.

**And there is a cheaper test that has not been tried and would be diagnostic in one run.** `D287` established that
plain `ping` and `ssh` work from these nodes while harness-launched processes are refused. **A full run in which A's
back-connect is replaced by an `ssh` to node1 on that port would separate "this process cannot connect outbound" from
"this port cannot be reached"** - and it is one command, where a packet capture is an afternoon.
...
## D381 — D380 is withdrawn as well: timeout is not the discriminator, and the honest position is that this leg is flaky

**The full configuration under `timeout 300`, which is the same launcher that connected twenty minutes earlier:**

    [back] connecting to 192.168.18.27:47702 as source
    [back] first connect failed: No route to host (65)
    [back] gave up after 450 attempts: No route to host (65)

**`timeout` is not the discriminator, and `D380` is withdrawn.** The run that connected used `timeout 20`; this one
used `timeout 300`; **and the failure occurred at the third second, long before either duration could matter.** The
success was luck.

**That is five withdrawn conclusions in six rounds** - `D371`, `D374`, `D376`, `D378`'s implicit "the invocation is
the variable", and now `D380`. **Every one of them was a plausible mechanism fitted to a small sample, and every one
was recorded with more confidence than the sample supported.** The single statistic that has survived is `D378`'s:
**the connect succeeds in some runs and fails in others with nothing this record can vary to predict which.**

**And the record has already said what to do about that, twice.** `D377`: *"the instrument to use is not `lsof` and
not an environment variable - it is a packet capture or the kernel's own connection log."* `D378`: the same, with the
reason - **`EHOSTUNREACH` is generated by the kernel, so only the kernel's record can distinguish a SYN that was
never sent from one that was refused, and those are the two remaining possibilities.**

**So this round stops rather than producing a sixth hypothesis.** Five rounds of process-level bisection have spent
the budget for that approach; **the next step is a different instrument, and it is a bigger one** - either
`log stream --predicate 'process == "TinyTitanCLI"'` during a run, or a packet capture on both nodes, both of which
this session has not attempted and neither of which is a twenty-second command.

**Where the objective stands, plainly and without a thesis.**

  * **A1-A5 are built, gated and green**: the layer range is bit-identical when unset, the exactness gate passes with
    **0 of 2048 elements differing**, the frame and the hooks are tested, and the suite is **1652 tests in 8 binaries
    with 0 failures**;
  * **the forward edge is proven across two machines**, with a real hidden state crossing and producing the correct
    first token (`D358`);
  * **the reverse edge is written, called and tested**, and connects in **six of eleven observed attempts**;
