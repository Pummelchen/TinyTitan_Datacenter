# TinyTitan v4.5 — ANE prefill

> Historical design record. ANE prefill shipped here as opt-in and was promoted
> to **on by default** in v4.6: `RuntimePrefillANE.environmentValue` returns
> `.on` when `TINYTITAN_PREFILL_ANE` is unset, and the runner falls back to the GPU
> quietly when no sidecar is present. Read the "off by default" statements below
> as the state at v4.5, not as current behaviour; the shipped default is in
> Runtime Controls.
...
format, the KV cache, the server API, and all of decode are untouched. At v4.5
it was opt-in — nothing changed without `TINYTITAN_PREFILL_ANE=on`; from v4.6 it is
the default and `TINYTITAN_PREFILL_ANE=off` opts out.
...
TINYTITAN_PREFILL_ANE=on .build/release/TinyTitanServer --model ... # or TinyTitanCLI
```

`TINYTITAN_PREFILL_ANE` accepts `off|on` and fails closed on anything else. With
...
arm (`benchmark/tinytitan_ane_prefill_ab.py`):
...
low-probability positions. That is why v4.5 shipped it off and why
`tools/golden-baseline.sh` still runs with `TINYTITAN_PREFILL_ANE=off`: it is the
only check that exercises real inference, and it must not move with an
experiment. v4.6 promoted the path to default on its own qualification; the
baseline deliberately pins the switch rather than following the default.
