// Which node owns which expert — the artifact the reference does not have.
//
// **Provenance.** Taken from the sister project `TinyTitan_Datacenter` (MIT), which designed this as its
// `D20`, and placed here because the reference has no shard concept of any kind: a grep for
// `shardPlan|ShardPlan|shard-plan|nodeID|nodeId` across this tree returns nothing. It lives in
// `TinyTitanDecodeProtocol` because it is a contract **between nodes**, which is what that module is for —
// every node must agree on it before a run, and the digest is how they find out that they do not.
//
// The design decisions, kept intact because each answers a specific failure:
//
// * `owners` is a flat array indexed by expert id, not a list per node, so an expert **cannot** be owned twice
//   and cannot be quietly unowned. The alternative makes both mistakes representable.
// * The plan is **data, not code**, because an agreement inside a compiled binary cannot be inspected,
//   diffed or frozen.
// * `canonicalDigest` is compared at bring-up, so two nodes that disagree **refuse to start** rather than
//   producing a wrong answer that looks like a numerics problem.
//
// Adapted only in its imports and its home; the logic is unchanged.
