...
kernel void moe_phase2_down_reduce_k8_remote(
    device const RoutedBlobs& routed [[buffer(0)]],
    constant ExpertOffsets& routed_offsets [[buffer(1)]],
    device const half* acts [[buffer(2)]],
    device const half* routing_w [[buffer(3)]],
    ...
    constant uint& F [[buffer(7)]],
    device const uint* io_status [[buffer(8)]],
    // The peers' contributions to this layer's slots, [d][8] fp32, zero where a peer owns nothing.
    // ALWAYS BOUND on this path: the single-node path uses the original kernel above instead of
    // relying on an unbound argument being null, which Metal does not promise.
    ...
    const float value = moe_int4_gemv_row_simd_dev_vec(
        dW, dS, dB, act_slot, d, FF, lane);
    if (lane == 0) {
        // Added BEFORE the ordered sum below and not after it: reducing node-level results on the host
        // associates the additions differently, and a different association is a different fp32 number.
        partial[sg_idx] = float(routing_w[sg_idx]) * value + remote[d * 8 + sg_idx];
    }
    threadgroup_barrier(mem_flags::mem_threadgroup);
